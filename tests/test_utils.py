...
import utils
from utils import debug_log, get_claude_path, get_state_file, load_shown, read_hook_input, save_shown


def test_get_claude_path():
    path = get_claude_path("test_file.log")
    assert path == os.path.expanduser("~/.claude/test_file.log")


def test_get_state_file_sanitizes_session_id_path_traversal():
    path = get_state_file("../../../etc/passwd", "seal_guard_state")
    prefix = get_claude_path(".seal_guard_state_")
...
def test_get_state_file_preserves_safe_session_id_characters():
    ...
    assert path == get_claude_path(".seal_guard_state_abc-DEF_123.json")
